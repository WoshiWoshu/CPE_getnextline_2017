...
static char *check_buffer_null(char *buffer)
{
	if (buffer == NULL)
		return (NULL);
	if (buffer[0] == 0)
...
static void cpy_str2_to_result(char *str2, char *result, int j)
{
	int i = -1;
...
static int cpy_str1_to_result(char *str1, char *result)
{
	int i = -1;
	int j = -1;
